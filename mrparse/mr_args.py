...
def mrparse_argparse(parser):
    ...
    sg.add_argument('--database', help='Database to search', default='all', choices=['all', 'pdb', 'afdb', 'esmfold'])
    sg.add_argument('--esmatlas', action='store_true', help=argparse.SUPPRESS)
    sg.add_argument('--afdb_seqs_directory', help=argparse.SUPPRESS)
    sg.add_argument('--esmatlas_seqs_directory', help=argparse.SUPPRESS)
    #sg.add_argument('--esmatlas_seqdb', help="Location of alternative ESMAtlas sequence database. To search the entire ESMAtlas database, download the latest sequence listing here: http://ftp.ebi.ac.uk/pub/databases/metagenomics/peptide_database/current_release/mgy_clusters.fa.gz and set path to file using this option. Note: very large file size!")
    sg.add_argument('--esmatlas_seqdb', help=argparse.SUPPRESS)
